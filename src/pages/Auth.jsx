  ...
  return (
    <div style={{ marginBottom:'14px' }}>
      ...
      <input type={type} placeholder={placeholder} value={value} onChange={onChange}
        ...
        style={{ width:'100%', padding:'10px 13px', fontSize:'14px', border:`1.5px solid ${foc ? C.accent : C.border}`,
...
export default function Auth() {
  ...
  const s = {
    root:{ minHeight:'100vh', display:'flex', fontFamily:"'Inter','Segoe UI',sans-serif", background:C.surface },
    left:{ width:'45%', background:`linear-gradient(145deg,${C.ink} 0%,${C.slate} 60%,#0D3B33 100%)`,
      display:'flex', flexDirection:'column', justifyContent:'space-between', padding:'48px', position:'relative', overflow:'hidden' },
    right:{ flex:1, display:'flex', alignItems:'center', justifyContent:'center', padding:'48px' },
    card:{ width:'100%', maxWidth:'400px' },
    tabs:{ display:'flex', background:C.border, borderRadius:'10px', padding:'3px', marginBottom:'24px' },
    tab:{ flex:1, padding:'8px', textAlign:'center', fontSize:'13px', fontWeight:'600', borderRadius:'8px', cursor:'pointer', border:'none' },
    btn:{ width:'100%', padding:'12px', background:`linear-gradient(135deg,${C.accent},${C.accentL})`,
      color:'#fff', border:'none', borderRadius:'8px', fontSize:'14px', fontWeight:'700', cursor:'pointer', marginTop:'6px' },
    googleBtn:{ width:'100%', padding:'10px', background:C.card, border:`1.5px solid ${C.border}`,
      borderRadius:'8px', fontSize:'13px', fontWeight:'600', color:C.ink, cursor:'pointer',
      display:'flex', alignItems:'center', justifyContent:'center', gap:'8px' },
    error:{ background:'#FEF2F2', border:'1px solid #FECACA', borderRadius:'8px', padding:'10px 14px', color:C.danger, fontSize:'13px', marginBottom:'14px' },
    success:{ background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:'8px', padding:'10px 14px', color:'#166534', fontSize:'13px', marginBottom:'14px' },
  }
  ...
  return (
    <div style={s.root}>
      ...
      <div style={s.right}>
        <div style={s.card}>
          <div style={{ fontSize:'24px', fontWeight:'800', color:C.ink, marginBottom:'6px' }}>{tab==='login' ? 'Welcome back' : 'Create your account'}</div>
          <div style={{ color:C.muted, fontSize:'14px', marginBottom:'28px' }}>{tab==='login' ? 'Sign in to manage your invoices.' : 'Join Kenyan businesses tracking payments smarter.'}</div>
          <div style={s.tabs}>
            <button style={{ ...s.tab, ...(tab==='login' ? { background:C.card, color:C.ink, boxShadow:'0 1px 4px rgba(0,0,0,0.1)' } : { background:'transparent', color:C.muted }) }} onClick={() => { setTab('login'); clear() }}>Sign in</button>
            <button style={{ ...s.tab, ...(tab==='signup' ? { background:C.card, color:C.ink, boxShadow:'0 1px 4px rgba(0,0,0,0.1)' } : { background:'transparent', color:C.muted }) }} onClick={() => { setTab('signup'); clear() }}>Create account</button>
          </div>
          ...
          {tab === 'login' ? (
            <>
              ...
              <div style={{ textAlign:'right', marginTop:'-6px', marginBottom:'14px' }}><span style={{ fontSize:'12px', color:C.accent, cursor:'pointer', fontWeight:'600' }}>Forgot password?</span></div>
              <button style={{ ...s.btn, opacity:loading?0.7:1 }} onClick={handleLogin} disabled={loading}>{loading ? 'Signing in...' : 'Sign in →'}</button>
            </>
          ) : (
            <>
              ...
              <button style={{ ...s.btn, opacity:loading?0.7:1 }} onClick={handleSignup} disabled={loading}>{loading ? 'Creating account...' : 'Create account →'}</button>
            </>
          )}
          ...
          <div style={{ marginTop:'20px', textAlign:'center', fontSize:'12px', color:C.muted }}>
