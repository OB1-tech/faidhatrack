...
export default function Dashboard({ session }) {
  ...
  const markPaid = async (id, current) => {
    ...
    const { error } = await sb.from('invoices').update({ status: next }).eq('id', id)
    if (error) { showToast('❌ ' + error.message); return }
  ...
  const S = {
    page:{ display:'flex', minHeight:'100vh', fontFamily:"'Inter','Segoe UI',sans-serif", background:'#F7F8FA', fontSize:'14px' },
    sidebar:{ width:'220px', background:'#1A1A2E', display:'flex', flexDirection:'column', padding:'24px 0', flexShrink:0 },
    sLogo:{ display:'flex', alignItems:'center', gap:'10px', padding:'0 20px 24px', borderBottom:'1px solid rgba(255,255,255,0.08)' },
    sLogoIcon:{ width:'32px', height:'32px', background:'linear-gradient(135deg,#0F7B6C,#13A88F)', borderRadius:'7px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'16px' },
    nav:{ padding:'16px 12px', display:'flex', flexDirection:'column', gap:'3px', flex:1 },
    navSection:{ color:'#475569', fontSize:'10px', fontWeight:'600', letterSpacing:'1.5px', textTransform:'uppercase', padding:'12px 10px 5px' },
    navItem:{ display:'flex', alignItems:'center', gap:'10px', padding:'9px 10px', borderRadius:'8px', color:'#94A3B8', fontSize:'13px', fontWeight:'500', cursor:'pointer', border:'none', background:'none', width:'100%', textAlign:'left' },
    navActive:{ background:'rgba(15,123,108,0.25)', color:'#fff' },
    sFooter:{ padding:'14px 12px 0', borderTop:'1px solid rgba(255,255,255,0.08)' },
    ...
    statCard:{ background:'#fff', border:'1px solid #E5E7EB', borderRadius:'12px', padding:'18px', position:'relative', overflow:'hidden' },
    bottomGrid:{ display:'grid', gridTemplateColumns:'1fr 320px', gap:'18px' },
  ...
  return (
    <div style={S.page}>
      ...
      <aside style={S.sidebar}>
        <div style={S.sLogo}>
          <div style={S.sLogoIcon}>🧾</div>
          <div>
            <div style={{ color:'#fff', fontSize:'15px', fontWeight:'700' }}>FaidhaTrack</div>
            <div style={{ color:'#13A88F', fontSize:'9px', letterSpacing:'1.5px', textTransform:'uppercase' }}>Invoice Manager</div>
          </div>
        </div>
        <nav style={S.nav}>
          <div style={S.navSection}>Main</div>
          <button style={{ ...S.navItem, ...S.navActive }}><span>📊</span> Dashboard</button>
          <button style={S.navItem} onClick={() => showToast('Invoices page — Day 3!')}><span>🧾</span> Invoices</button>
          <button style={S.navItem} onClick={() => showToast('Clients page — Day 3!')}><span>👥</span> Clients</button>
          <button style={S.navItem} onClick={() => showToast('Payments — Day 3!')}><span>💳</span> Payments</button>
          <div style={S.navSection}>Tools</div>
          <button style={S.navItem} onClick={() => showToast('AI Insights — Day 4!')}><span>🤖</span> AI Insights</button>
          <button style={S.navItem} onClick={() => showToast('Reports coming soon!')}><span>📈</span> Reports</button>
          <button style={S.navItem} onClick={() => sb.auth.signOut()}><span>🚪</span> Sign out</button>
        </nav>
        <div style={S.sFooter}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', padding:'8px 10px' }}>
            <div style={{ width:'30px', height:'30px', background:'linear-gradient(135deg,#0F7B6C,#F4A020)', borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center', color:'#fff', fontSize:'11px', fontWeight:'700' }}>{userInit}</div>
            <div>
              <div style={{ color:'#fff', fontSize:'12px', fontWeight:'600' }}>{userName}</div>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
              ...
              <div style={{ background:'linear-gradient(135deg,#0D3B33,#16213E)', borderRadius:'12px', padding:'16px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'12px' }}>
                  ...
                  <div>
                    <div style={{ color:'#fff', fontSize:'13px', fontWeight:'700' }}>AI Cash Flow Insights</div>
                ...
                {[
                  ...
                  invoices.length ? `${Math.round((paid.length/invoices.length)*100)}% of your invoices have been paid.` : 'Create your first invoice to get insights.',
                ].map((tip, i) => (
          ...
          <div style={S.modal}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'12px' }}>
              ...
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <input style={S.fi} placeholder="Web design services — June 2026" value={form.desc} onChange={e => setForm({...form, desc:e.target.value})} />
              </div>
            </div>
            <div style={{ display:'flex', gap:'10px', marginTop:'18px' }}>
              <button style={{ ...S.btnPrimary, flex:1, opacity:saving?0.7:1 }} onClick={saveInvoice} disabled={saving}>
